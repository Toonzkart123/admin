...
// import { Search, Filter, UserPlus, Download, Mail, Lock, Edit, Trash2, UserCheck, Phone, Eye } from 'lucide-react';
...
//         const response = await fetch('https://backend-lzb7.onrender.com/api/admin/users', {
//           headers: {
...
//           orders: user.orderHistory ? user.orderHistory.length : 0,
//           totalSpent: calculateTotalSpent(user),
//           lastActive: user.lastUpdated || user.createdAt
//         }));
...
//   // Helper function to map backend roles to UI roles
...
//   // Helper function to map UI roles back to backend roles
...
//   // Helper function to calculate total spent (placeholder)
//   const calculateTotalSpent = (user) => {
//     // This would ideally come from the order history
//     return user.orderHistory && user.orderHistory.length > 0 ? user.orderHistory.length * 50 : 0;
//   };
...
//         (user.id && user.id.toString().toLowerCase().includes(lowercaseQuery))
//       );
...
//         status: user.status,
//         // We don't include password when editing
//       });
//       setShowUserForm(true);
//     }
//   };
...
//         const response = await fetch(`https://backend-lzb7.onrender.com/api/admin/users/${userId}`, {
//           method: 'DELETE',
//           headers: {
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//         response = await fetch(`https://backend-lzb7.onrender.com/api/admin/users/${selectedUser.id}`, {
//           method: 'PUT',
//           headers: {
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//       } else {
//         // Add new user
//         response = await fetch('https://backend-lzb7.onrender.com/api/admin/users', {
//           method: 'POST',
//           headers: {
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//       if (!response.ok) {
//         throw new Error(`Failed to ${selectedUser ? 'update' : 'add'} user: ${response.status} ${response.statusText}`);
//       }

//       const responseData = await response.json();
      
//       // Refresh user list
//       const refreshResponse = await fetch('https://backend-lzb7.onrender.com/api/admin/users', {
//         headers: {
//           'Authorization': `Bearer ${token}`,
//           'Content-Type': 'application/json'
//         }
...
//         throw new Error(`Failed to refresh users: ${refreshResponse.status} ${refreshResponse.statusText}`);
//       }

//       const refreshData = await refreshResponse.json();
      
//       // Transform and update users
//       const transformedUsers = refreshData.map(user => ({
//         id: user._id,
//         name: user.name,
//         email: user.email,
//         phone: user.phone || 'N/A',
...
//       setUsers(transformedUsers);
//       applyFilter(activeTab, transformedUsers);
...
//                         <option value="store">Support</option>
//                       </select>
//                     </div>
...
const Users = () => {
  ...
  const applyFilter = (filter, userList = users) => {
    ...
    if (filter === 'all') {
      ...
      setFilteredUsers(userList.filter(user => user.role === 'Store'));
    } else if (filter === 'active') {
      setFilteredUsers(userList.filter(user => user.status === 'Active'));
    } else if (filter === 'inactive') {
      setFilteredUsers(userList.filter(user => user.status === 'Inactive'));
              ...
              <div className="bg-white px-6 pt-6 pb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-1">
                    ...
                    {(selectedUser.role === 'Admin' || selectedUser.role === 'Store') && (
                      <div className="pt-4">
                        <h4 className="text-sm font-medium text-gray-500 mb-3">Permissions</h4>
                        <div className="space-y-3">
                          <div className="flex items-center">
